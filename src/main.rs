...
mod state {
    ...
    lazy_static! {
        static ref UNDERLINE_REGEX: Regex = Regex::new(r"\{\{.*?\}\}").expect("Invalid regex for underline");
        static ref TEMPLATE_VAR_REGEX: Regex = Regex::new(r"\{\{([^}]+?)\}\}").expect("Invalid regex for template variables");
        static ref IMAGE_WIDTH_CONTROL_REGEX: Regex = Regex::new(r"!\{\(.*?\)\}\[(.*?)\]\((.*?)\)").expect("Invalid regex for image width control");
    }
...
", filename, file_path.to_string_lossy()));
        ...
        fn calculate_editor_metrics(&self, ui: &mut egui::Ui) -> (f32, f32, f32, f32) {
            ...
            (content_height, visible_height, line_number_width, char_width)
        }
        ...
        pub fn show_panels(&mut self, ctx: &egui::Context) {
            egui::CentralPanel::default().show(ctx, |ui| {
                ...
                let (editor_content_height, editor_visible_height, line_number_width, _char_width) = self.calculate_editor_metrics(ui);
                ...
                if let Some(char_range) = state.cursor.char_range() {
                    ...
                    if primary_idx != secondary_idx {
                        ...
                        let selected_text = &self.markdown_text[start_byte..end_byte];
                            ...
                            for cap in IMAGE_WIDTH_CONTROL_REGEX.captures_iter(selected_text).collect::<Vec<_>>().into_iter().rev() {
